...
def build_insight_prompt(user: dict, history: list, 
    ...
    """
    Builds the dynamic prompt for Claude-Sonnet-4.
    Removes hardcoded fallbacks to reflect actual student behavior.
    """
    name = user.get("name", "the student")

    # 1. Summarize actual history from the last 14 days
    recent = history[:14]
    n = max(len(recent), 1)

    # REMOVED: Hardcoded defaults. We now use 0 as a neutral fallback 
    # if data is missing, though your new checkin logic ensures these exist.
    avg_sleep    = round(sum(e.get("sleep_hours", 0) for e in recent) / n, 1)
    avg_wake     = round(sum(e.get("wake_time", 0) for e in recent) / n, 1)
    avg_gap      = round(sum(e.get("performance_gap", 0) for e in recent) / n, 1)
    ...
    # 2. Summarize detected shifts (Passed from your dynamic detection logic)
    shift_text = ""
    if shifts:
    ...
    # 3. Semester week context (Cleaned up for dynamic readability)
    week_map = {
        (1, 2):   "Early semester — baseline forming, optimism typically high.",
        (3, 5):   "Mid-early semester — workload picking up, first stress signals common.",
        (6, 8):   f"Week {week_number} — the 'Midterm Peak'. Documented stress peak. Sunlight often decreasing.",
        (9, 12):  "The 'Dark Stretch' — Motivation historically lowest, finals approaching.",
        (13, 16): "Finals period — Peak academic stress. Adrenaline and exhaustion collide."
    }
    
    week_note = "Standard semester week."
    for range_tuple, note in week_map.items():
        if range_tuple[0] <= week_number <= range_tuple[1]:
            week_note = note
            break
...
THEIR ACTUAL BEHAVIOR (LAST 14 DAYS):
- Average sleep:         {avg_sleep} hours
- Average wake time:     {avg_wake} (decimal format)
- Classes attended:      {pct_attended}%
- Left room/building:    {pct_left}%
- Ate proper meals:      {pct_ate}%
- Performance gap (1-5): {avg_gap} (how much they are 'masking' their real state)

{shift_text}

YOUR RULES — READ CAREFULLY:
1. You are NOT a medical tool. Never diagnose, never use clinical terms.
2. Never say: depression, anxiety disorder, mental illness, crisis, psychiatric, therapy.
3. DO say: patterns, shifts, baseline, what the data shows, what research tells us.
4. Speak directly to {name} in second person — warm, specific, human.
5. Reference their ACTUAL numbers above — no generic advice.
6. Acknowledge the semester week context — validate that what they feel is predictable.
